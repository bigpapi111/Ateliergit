...
QSqlQueryModel* invite::chercher_invite(QString rech){
    QString sQuery="SELECT cin,nom,prenom,metier, age, password FROM inviter WHERE nom LIKE'%"+rech+"%' or prenom LIKE'%"+rech+"%'or metier LIKE'%"+rech+"%'or age LIKE'%"+rech+"%'or password LIKE'%"+rech+"%' ";
...
bool invite::modifier()
{
 ...
 QString age_string= QString::number(age);
 QString password_string= QString::number(password);
     query.prepare("UPDATE inviter SET nom=:nom , prenom=:prenom , metier=:metier, age=:age, password=:password "
                   " where cin= :cin");
     query.bindValue(":cin",cin_string);
     query.bindValue(":nom",nom);
     query.bindValue(":prenom",prenom);
